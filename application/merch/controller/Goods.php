...
class Goods extends Base
{
	...
	protected function grouppost()
	{
		...
		$merch = $this->merch;
		if (!empty($id)) {
			$item = Db::name('shop_goods_group')->where('id',$id)->where('merchid',$merch['id'])->find();
		...
		if (Request::instance()->isPost()) {
			...
			$data = array('displayorder' => $displayorder, 'name' => $groupname, 'merchid' => $merch['id'], 'goodsids' => implode(',', $goodsids), 'enabled' => $enabled);
			...
			show_json(1, array('url' => url('merch/goods/groupedit', array('id' => $id))));
		}
	...
	protected function goodslist($goodsfrom = 'sale')
	{
		...
		$this->assign(['list'=>$list,'pager'=>$pager,'category'=>$category,'cate'=>$cate,'status'=>$status,'keyword'=>$keyword,'goodsfrom'=>$goodsfrom,'totals'=>$totals]);
