...
class Community extends Base
{
	...
	{
		$list = Db::name('community')->where('status = 1 and deleted = 0')->field('id,communityname')->select(); 
        foreach ($list as &$val) {
	...
	public function housepropertyorder()
	{
		...
		$bill['fees'] = $othermoney ? $othermoney : array();
		...
		$extra = array(array('name'=>'手续费','value'=>number_format($poundage,2)));
		$result['bill']['extra'] = $extra;
		$result['bill']['totalprice'] = number_format($totalprice,2);
		$this->result(1,'success',$result);
	}
	...
	public function repair()
	{
		...
		$data['repairsn'] = model('common')->createNO('community_apply_repair','repairsn','RE');
	...
	public function pay()
	{
		...
		if ($paytype == 1) {
			...
			if (isset($set['pay']) && ($set['pay']['app_wechat'] == 1)) {
				$wechat = model('payment')->wechat_build($params, $headerinfo['device-type'], 123, 'app');
				if (is_error($wechat)) {
		...
		if($paytype == 2) {
			if (isset($set['pay']) && ($set['pay']['app_alipay'] == 1)) {
				...
				$alipay = model('payment')->alipay_build($params, $headerinfo['device-type'], 123, getHttpHost() . '/public/dist/order','app');
				if (empty($alipay)) {
	...
	public function order()
	{
		...
		$page = input('page/d',1);
		$pagesize = input('pagesize/d',10);
		$type = input('type/s','water');
		if(!in_array($type, array('water','electricity','property'))) {
			$type = 'water';
		}
		$list = Db::name('community_apply_payment')->where('status > 0 and mid = ' . $mid . ' and type = "' . $type . '"')->order($page,$pagesize)->page($page,$pagesize)->select();
		foreach ($list as &$val) {
			$val['extra'] = array(array('name'=>'手续费','value'=>$val['poundage']));
			$val['house'] = Db::name('community_house')->alias('h')->join('community c','c.id = h.communityid')->join('community_building b','b.id = h.buildingid')->where('h.id = ' . $val['houseid'])->field('h.*,c.communityname,b.buildingname')->find();
			...
			if($val['type'] == 'water') {
				...
				if($val['type'] == 'property') {
					...
					foreach ($orders as &$value) {
						$value['othermoney'] = iunserializer($value['othermoney']);
						$value['price'] = $value['propertymoney'];
					}
					unset($value);	
				}
			}
			...
			if($val['status'] == -1) {
				$statusstr = '已取消';	
				$log[0] = array('type' => -1, 'time' => $val['canceltime'], 'remark' => '取消订单或已处理完成主动关闭');
			} else {
				if($val['status'] == 0) {
					$statusstr = '待支付';
					$log[0] = array('type' => 0, 'time' => $val['createtime'], 'remark' => '等待支付');
				} else {
					if($val['status'] == 1) {
						$statusstr = '待处理';
						$log[] = array('type' => 1, 'time' => $val['paytime'], 'remark' => '付款成功，等待处理');
						$log[] = array('type' => 0, 'time' => $val['createtime'], 'remark' => '等待支付');
					} else {
						if($val['status'] == 2) {
							$statusstr = '已完成';
							$log[] = array('type' => 2, 'time' => $val['finishtime'], 'remark' => '已完成缴费');
							$log[] = array('type' => 1, 'time' => $val['paytime'], 'remark' => '付款成功，等待处理');
							$log[] = array('type' => 0, 'time' => $val['createtime'], 'remark' => '等待支付');
						}
					}
